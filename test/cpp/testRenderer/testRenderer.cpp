#include "TestRenderer.h" // Твой заголовочный файл с EXPORT и PLYGaussianPoint
#include <windows.h>
#include <GL/glew.h>
#include <GL/gl.h>
#include <cstdio>
...
static void LoadHookFunctions() { if (GetMatrixByLocation) return; HMODULE dll = GetModuleHandleA("testHookDLL.dll"); if (dll) { GetMatrixByLocation = (PFN_GET_MATRIX_BY_LOC)GetProcAddress(dll, "GetMatrixByLocation"); if (GetMatrixByLocation) LogRenderer("Found GetMatrixByLocation."); else LogRenderer("ERR: GetMatrixByLocation not found."); } else LogRenderer("ERR: Hook DLL not found."); }
